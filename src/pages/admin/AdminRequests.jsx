...
import { Loader2, CheckCircle, XCircle, Clock, Calendar, MapPin, User, Mail, BookOpen, AlertCircle, Building2 } from 'lucide-react';
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-5 shadow-sm group-hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-4">
                ...
                <div className="flex-1">
                  {/* Requester Details (Now at Top) */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-lg shadow-sm border border-blue-200">
                        {item.faculty?.name?.charAt(0) || "U"}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-slate-800">{item.faculty?.name}</span>
                          {item.faculty?.designation && (
                            <span className="text-[10px] uppercase font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded-md border border-slate-200">{item.faculty.designation}</span>
                          )}
                        </div>
                  ...
                  <h3 className="text-xl font-black text-slate-900 group-hover:text-blue-700 transition tracking-tight">
                    {item.isBatch
                      ? (item.isMultiVenue ? `Group Booking (${item.items.length} Items)` : item.items[0].venue?.name)
                      : item.venue?.name}
                  </h3>

                  {item.isBatch && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {[...new Set(item.items.map(req => req.venue?.name))].map((vName, idx) => (
                        <span key={idx} className="px-2.5 py-1 bg-indigo-50 text-indigo-700 text-xs font-bold rounded-lg border border-indigo-100 flex items-center gap-1 shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-5 text-sm text-slate-600 mt-4 font-medium">
                    {!item.isBatch && (
                      <span className="flex items-center bg-slate-50 px-2 py-1 rounded-md border border-slate-100">
                        <MapPin className="w-4 h-4 mr-1.5 text-blue-500 group-hover:scale-110 transition" />
                        {item.venue?.location}
                      </span>
                    )}
                    ...
                    {item.isBatch ? (
                      <div className="w-full mt-2 space-y-1 bg-blue-50/50 p-3 rounded-xl border border-blue-100">
                        <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1">Booked Slots</p>
                        {item.items.map((sub, idx) => (
                          <div key={idx} className="flex items-center text-xs text-blue-900 font-bold">
                            <Clock className="w-3.5 h-3.5 mr-2 text-blue-400" />
                            {sub.timeSlot} {item.isMultiVenue && <span className="ml-1 text-blue-400 font-normal">at {sub.venue?.name}</span>}
                          </div>
                        ))}
                      </div>
                    ) : (
                      <span className="flex items-center bg-blue-50 text-blue-900 px-3 py-1 rounded-md border border-blue-100 font-bold">
                        <Clock className="w-4 h-4 mr-1.5 text-blue-500 group-hover:scale-110 transition" />
                        {item.timeSlot}
                      </span>
                    )}
                  </div>
